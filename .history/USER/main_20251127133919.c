...
//***************串口3初始化为9600       for 设备内部通信准备******//
	...
	u1_printf("Short PA2(TX) <-> PA3(RX), then watch for RX data\n");
	
	/* 诊断: 检查USART2状态 */
	u1_printf("USART2 CR1=0x%04X\n", USART2->CR1);
	u1_printf("USART2 SR=0x%04X\n", USART2->SR);
	u1_printf("NVIC ISER[1]=0x%08X\n", NVIC->ISER[1]);
	u1_printf("PA3 level=%d\n", GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_3));
	...
	while(1)
	{
		...
		SysTick_Delay_ms(200);  /* 缩短延时，避免看门狗超时 */
		IWDG_Feed();
		
		/* 检查RXNE标志 (硬件层面) */
		if(USART2->SR & 0x0020)  /* RXNE bit */
		{
			u1_printf("RXNE=1! DR=0x%02X\n", USART2->DR);
		}
		
		/* 检查是否收到回环数据 (软件层面) */
		if(U2_Inf.Recive_Ok_Flag)
		{
			u1_printf("RX OK! Len=%d\n", U2_Inf.RX_Length);
