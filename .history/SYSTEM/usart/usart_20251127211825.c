								...
								if(Address == 1000)
									{
										...
										uartSendDma(&uartDebugHandle, U1_Inf.TX_Data,7);
...
uint8 A1B1_Jizu_ReadResponse(uint8 address)
{
	...
	uartSendDma(&uartDebugHandle, U1_Inf.TX_Data,Bytes +5);
								   ...
								   if(Length == 11)  //��ߵ����ݳ���Ҳ�����ܱ仯�������ֽڳ��ȵı仯
									   {
										   ...
										   uartSendDma(&uartDebugHandle, U1_Inf.TX_Data,27);
...
uint8 ModuBus1RTU_WriteResponse(uint16 address,uint16 Data16)
{
	...
	uartSendDma(&uartDebugHandle, U1_Inf.TX_Data,8);
