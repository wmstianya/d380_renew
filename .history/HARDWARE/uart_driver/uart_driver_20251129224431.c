 ...
 * @brief  DMA发送完成中断处理
 * @param  handle: UART句柄指针
 * @note   DMA TC表示数据已搬到DR，但最后一字节仍在移位寄存器
 *         需要等待USART TC中断才是真正发送完成
 */
void uartDmaTxIrqHandler(UartHandle* handle)
{
    ...
    if (DMA_GetITStatus(handle->config.dmaTxTcFlag) != RESET)
    {
        ...
        /* 不要在这里清除txBusy! 等待USART TC中断 */
        /* 启用USART发送完成中断 */
        USART_ITConfig(handle->config.usartx, USART_IT_TC, ENABLE);
    }
}


/**
 * @brief  USART发送完成中断处理 (移位寄存器发送完毕)
 * @param  handle: UART句柄指针
 * @note   在USART中断服务函数中调用，表示数据真正发送完成
 */
void uartTxCompleteHandler(UartHandle* handle)
{
    ...
    /* 检查USART TC中断 */
    if (USART_GetITStatus(handle->config.usartx, USART_IT_TC) != RESET)
    {
        /* 清除TC中断标志 */
        USART_ClearITPendingBit(handle->config.usartx, USART_IT_TC);
        
        /* 禁用TC中断 (只需触发一次) */
        USART_ITConfig(handle->config.usartx, USART_IT_TC, DISABLE);
...
uint8_t uartDisplayInit(uint32_t baudRate)
{
    ...
    uartDisplayHandle.config.nvicPrePriority= 2;  /* 优先级3/4 - 显示屏 */
